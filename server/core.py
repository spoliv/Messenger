...
class MessageProcessor(threading.Thread, metaclass=ServerVerifier):
    ...
    def proc_client_message(self, message, client):
        ...
        # Если получатель сообщения all. т.е. в чат
        # elif ACTION in message and message[ACTION] == CHAT and DESTINATION in message and TIME in message \
        #         and SENDER in message and CHAT_TEXT in message and self.names[message[SENDER]] == client:
        #     if message[DESTINATION] == 'all':

        elif ACTION in message and message[ACTION] == CHAT and TIME in message \
                and SENDER in message and CHAT_TEXT in message and self.names[message[SENDER]] == client:
            self.database.save_chat_message(message[SENDER], message[CHAT_TEXT])
            print('Chat')
            try:
                send_message(client, RESPONSE_200)
            except OSError:
                self.remove_client(client)
        # Если это запрос на получение сообщений чата
        elif ACTION in message and message[ACTION] == GET_CHAT_MESSG and USER in message and \
                self.names[message[USER]] == client:
            response = RESPONSE_202
            response[LIST_INFO] = self.database.get_chat_messages()
            # print(type(response[LIST_INFO]))
            # print(response[LIST_INFO][0])
            try:
