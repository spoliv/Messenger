...
from sqlalchemy import create_engine, Table, Column, Integer, String, BLOB, MetaData, ForeignKey, DateTime, Text
...
class ServerStorage:
    ...
    # Класс - отображение таблицы истории чата
    class ChatHistory:
    ...
    def __init__(self, path):
        ...
        # Создаём таблицу фватаров пользователей
        users_avatars_table = Table(
            'Avatars', self.metadata, Column(
        ...
        # Создаём таблицу истории чата
        chat_history_table = Table('Chat_history', self.metadata,
                                   Column('id', Integer, primary_key=True),
        ...
        mapper(self.UserAvatars, users_avatars_table)
        mapper(self.ActiveUsers, active_users_table)
        mapper(self.LoginHistory, user_login_history)
        mapper(self.UsersContacts, contacts)
        mapper(self.UsersHistory, users_history_table)
        mapper(self.ChatHistory, chat_history_table)
        # Создаём сессию
    ...
    # Функция регистрации пользователя. Принимает имя и хэш пароля, создаёт
    ...
    def add_avatar(self, data):
        user_avatar = self.UserAvatars(data)
        self.session.add(user_avatar)
        self.session.commit()
    ...
    # Функция сохраняющяя в БД сообщения из чата
    def save_chat_message(self, contact, message):
        message_row = self.ChatHistory(contact, message)
        self.session.add(message_row)
        self.session.commit()

    # Функция возвращающая все сообщения из чата
    def get_chat_messages(self):
        # query = self.session.query(
        #     self.ChatHistory.contact,
...
if __name__ == '__main__':
    test_db = ServerStorage('../server_database.db4')
    test_db.save_chat_message('test1', 'Hello')
